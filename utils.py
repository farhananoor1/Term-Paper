...
def train(
    ...
    train_ds = dataset.load_folder("input/maestro/train/", window=window, fmt=model.data_fmt)
    train_dl = th.utils.data.DataLoader(train_ds, batch_size=batch_size_train, shuffle=True)
    test_ds = dataset.load_folder("input/maestro/test/", window=window, fmt=model.data_fmt)
...
if __name__ == "__main__":
    ...
    ds = dataset.load_folder("input/maestro/test", 256, ev)
